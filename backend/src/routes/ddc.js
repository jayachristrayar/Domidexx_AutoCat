...
router.post('/recommend', asyncHandler(async(req,res)=>{ const metadata=req.body?.metadata||req.body||{}; const decision=await recommendDdc(metadata); const row=await saveDdcDecision({userId:req.user.userId,metadata,decision}); res.status(201).json({id:row.id,decision}); }));
