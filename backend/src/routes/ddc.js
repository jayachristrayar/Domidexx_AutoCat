...
router.post('/recommend', asyncHandler(async(req,res)=>{
  ...
  // ever called (product spec section 6/17: extension -> backend ->
  // authorized provider, never extension -> provider directly).
  let model;
  try {
    model = resolveAuthorizedModel(req.user.modelAccess, req.body?.model);
  } catch (error) {
