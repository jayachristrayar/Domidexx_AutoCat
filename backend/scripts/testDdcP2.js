...
function rec(metadata){return recommendDdc(metadata)}
...
let d=rec({title:'Introduction to Library and Information Science',description:'An introductory whole-book treatment of librarianship, library services, information organization, cataloging, reference services and the functions of libraries.',table_of_contents:['The library as institution','Information organization','Reference and discovery services','Library operations']});
...
assert.strictEqual(rec({title:'Library Operations',description:'Cataloging, acquisitions, circulation, reference service, serials control, and discovery workflows.',table_of_contents:['Acquisitions','Cataloging','Circulation','Reference services']}).recommended_ddc.number,'025');
assert.strictEqual(rec({title:'Administration of Academic Libraries',description:'Library administration, staffing, planning and management of library services.',table_of_contents:['Library planning','Staffing','Service assessment']}).primary_subject,'Library & Information Science');
assert.strictEqual(rec({title:'Principles of Business Management',description:'Corporate planning, leadership, organization, and general management practices.'}).recommended_ddc.number,'658');
assert.strictEqual(rec({title:'Artificial Intelligence',description:'Machine learning, neural networks, model architecture, training, inference, algorithms and deep learning.'}).recommended_ddc.number,'006.3');

d=rec({title:'Artificial Intelligence Applications in Academic Libraries',description:'The book examines academic library services, discovery, reference, library operations, information literacy, and library management using AI as an enabling technology.',table_of_contents:['AI for reference services','Discovery in academic libraries','Library operations','Information literacy services','Managing AI-enabled library services'],keywords:['AI','Machine Learning','Python','Libraries','Automation']});
